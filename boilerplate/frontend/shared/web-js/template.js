...
import MultiFactorAuth from "supertokens-web-js/recipe/multifactorauth";`,
    webauthn: `import SuperTokens from "supertokens-web-js";
import Session from "supertokens-web-js/recipe/session";`,
...
export const generateWebJSTemplate = ({ configType, isFullStack, userArguments }) => {
    ...
    if (userArguments?.firstfactors || userArguments?.secondfactors) {
        ...
        if (factors.includes("webauthn"))
            recipesSet.add("webauthn");
        ...
        if (userArguments.secondfactors && userArguments.secondfactors.length > 0)
            recipesSet.add("multiFactorAuth");
        if (userArguments.secondfactors?.includes("totp"))
            recipesSet.add("totp");
        if (userArguments.secondfactors?.some((f) => f.includes("email")))
            recipesSet.add("emailVerification");
        if (configType === "multitenancy")
            recipesSet.add("multitenancy");
    }
    else {
        configToRecipes[configType]?.forEach((recipe) => {
    ...
    const hasMFA = (userArguments?.secondfactors && userArguments.secondfactors.length > 0) || recipes.includes("multiFactorAuth");
    ...
    const needsEmailVerification = hasMFA &&
