...
export const pyRecipeImports = {
    ...
    thirdParty: "from supertokens_python.recipe import thirdparty\nfrom supertokens_python.recipe.thirdparty.provider import ProviderInput, ProviderConfig, ProviderClientConfig",
...
export const pyRecipeInits = {
                    ...
                    config=ProviderConfig(
                        ...
                        clients=[
                            ProviderClientConfig(
                                ...
                                client_secret="${provider.clientSecret}"${provider.additionalConfig
        ? `,
                                additional_config=${JSON.stringify(provider.additionalConfig, null, "####").replace(new RegExp("####", "g"), "".padStart(36))}`
        : ""}
                            )
                        ]
                    )
    ...
    multiFactorAuth: (firstFactors, secondFactors) => {
        const firstFactorsStr = firstFactors?.map((factor) => `"${factor}"`).join(", ") || `"emailpassword", "thirdparty"`;
...
export const generatePythonTemplate = ({ configType, userArguments, framework }) => {
    ...
    if (userArguments?.firstfactors || userArguments?.secondfactors) {
        ...
        if (factors.includes("emailpassword"))
            recipesSet.add("emailPassword");
        if (factors.includes("thirdparty"))
            recipesSet.add("thirdParty");
        if (factors.some((f) => f.startsWith("otp-") || f.startsWith("link-")))
            recipesSet.add("passwordless");
    }
    else {
    ...
    if (recipes.includes("accountLinking")) {
        imports.push("from supertokens_python.recipe.accountlinking.types import AccountInfoWithRecipeIdAndUserId, ShouldAutomaticallyLink, ShouldNotAutomaticallyLink", "from supertokens_python.recipe.session.interfaces import SessionContainer", "from supertokens_python.types import User");
    ...
    if (hasMFA && userArguments?.secondfactors && userArguments.secondfactors.length > 0) {
        imports.push("from supertokens_python.recipe.multifactorauth.interfaces import RecipeInterface as MFARecipeInterface", "from supertokens_python.recipe.multifactorauth.types import MFARequirementList, FactorIds");
