            ...
            .map((p) => `{
                Config: tpmodels.ProviderConfig{
                    ...
                    Clients: []tpmodels.ProviderClientConfig{
                        {
                            ClientID:     "${p.clientId}",
                            ClientSecret: "${p.clientSecret}",${p.additionalConfig
            ? `
                            AdditionalConfig: map[string]interface{}{
                                ${Object.entries(p.additionalConfig)
                .map(([key, value]) => {
                const escapedValue = typeof value === "string" ? value.replace(/\n/g, "\\n") : value;
...
export const generateGoTemplate = ({ configType, userArguments, }) => {
    ...
    const recipes = configToRecipes[configType].filter((recipe) => recipe !== "accountLinking" && // accountLinking is used directly, not via init list
