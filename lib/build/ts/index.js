...
import { modifyAnswersBasedOnFlags, modifyAnswersBasedOnSelection, validateUserArguments, } from "./userArgumentUtils.js";
...
async function checkVersion() {
    const remoteVersionReq = await fetch("https://raw.githubusercontent.com/supertokens/create-supertokens-app/main/package.json");
    const remoteVersion = (await remoteVersionReq.json()).version;
    const localVersion = package_version;
    const comparison = compareVersions(localVersion, remoteVersion);
    switch (comparison) {
        case -1: {
            Logger.warn(`A new version of the SuperTokens CLI is available: ${remoteVersion}. Your local version is ${localVersion}.`);
...
async function printInformation() {
    ...
    console.log("\n");
    console.log(chalk.bold(`create-supertokens-app (v${package_version})`), "lets you quickly get started with using SuperTokens!\n");
    console.log("Choose your tech stack and the authentication method, we will create a working project that uses SuperTokens for you.");
    console.log("\n");
}
    ...
    try {
        ...
        */
        ...
        if (answers.recipe === "multitenancy" || answers.recipe === "multifactorauth") {
            ...
            if (errorPlaceholder !== "") {
                const errorMessage = `create-supertokens-app does not support ${recipePlaceholder} for ${errorPlaceholder} yet. ${answers.recipe === "multitenancy"
        ...
        if (e.skipGithubLink !== true) {
            Logger.error("If you think this is an issue with the tool, please report this as an issue at https://github.com/supertokens/create-supertokens-app/issues");
        }
        process.exit(1);
    }
}
