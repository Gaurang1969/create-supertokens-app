import { allBackends, // Add SupportedFrontends
allFrontends, // Re-add allFrontends
allPackageManagers, allRecipes, isValidBackend, isValidFrontend, // Re-add isValidFrontend
isValidPackageManager, isValidRecipeName, } from "./types.js";
...
export function validateUserArguments(userArguments) {
    ...
    if (userArguments.backend !== undefined) {
        ...
        if (!isValidBackend(normalizedBackend)) {
            const availableBackends = allBackends
                ...
                .map((e) => `    - ${e}`)
                .join("\n");
            throw new Error(`Invalid backend provided: "${userArguments.backend}". Valid values:\n${availableBackends}`);
        }
    ...
    if (userArguments.firstfactors !== undefined) {
        ...
        if (invalidFactors.length > 0) {
            throw new Error(`Invalid first factors provided: ${invalidFactors.join(", ")}. Valid values are: ${validFirstFactors.join(", ")}`);
        }
    }
    if (userArguments.secondfactors !== undefined) {
        ...
        if (invalidFactors.length > 0) {
            throw new Error(`Invalid second factors provided: ${invalidFactors.join(", ")}. Valid values are: ${validSecondFactors.join(", ")}`);
        }
    }
...
function validateProviders(providers) {
    ...
    if (invalidProviders.length > 0) {
        throw new Error(`Invalid providers provided: ${invalidProviders.join(", ")}. Valid values are: ${validProviderIds.join(", ")}`);
    }
}
...
export function modifyAnswersBasedOnFlags(answers, userArguments) {
    ...
    if (userArguments.recipe !== undefined) {
        ...
        if (userArguments.firstfactors?.includes("emailpassword") &&
    ...
    if (userArguments.backend !== undefined) {
        const selectedBackend = allBackends.find((b) => b.id === userArguments.backend || b.frameworks?.some((f) => f.id === userArguments.backend));
        const backendValue = selectedBackend?.id === userArguments.backend
            ? selectedBackend.id
...
export function getShouldAutoStartFromArgs(userArguments) {
    if (userArguments.autostart !== undefined &&
