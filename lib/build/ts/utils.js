...
import { getRecipesFromFactors } from "./types.js";
import fs from "fs";
...
export async function downloadExternalApp(locations, folderName) {
    ...
    await pipeline(downloadResponse.body, tar.extract({
        cwd: `./${folderName}`,
        strip: 1,
        strict: true,
    }, []));
...
async function performAdditionalSetupForFrontendIfNeeded(selectedFrontend, folderName, userArguments) {
    ...
    const doesWebJsExist = () => {
        ...
        const doesExistInAuthReact = fs.existsSync(`${sourceFolder}/node_modules/supertokens-auth-react/node_modules/supertokens-web-js`);
        ...
        if (stat.isDirectory()) {
            ...
            if (fileContent.includes("${jsdeliveryprebuiltuiurl}")) {
                if (actualBundleSource === "") {
                    ...
                    if (!response.ok) {
                        throw new Error("Failed to fetch the actual bundle source for pre built UI. Please try again in sometime");
                    }
...
export function checkMfaCompatibility(answers, userArguments) {
    const hasMFA = answers.recipe === "multifactorauth" || (userArguments.secondfactors && userArguments.secondfactors.length > 0);
    const isGoBackend = answers.backend !== undefined && (answers.backend.includes("go") || answers.backend === "go-http");
    if (hasMFA && isGoBackend) {
        throw new Error("Multi-factor authentication is not currently supported with the Go SDK. Please use a different backend or recipe.");
    }
}
...
function printDemoCoreWarning() {
    Logger.warn("This project is configured to use the public SuperTokens demo core at https://try.supertokens.com.\n" +
        ...
        "See https://supertokens.com/docs/deployment/self-host-supertokens for self-hosting, or https://supertokens.com/auth to sign up for the managed service.");
}
export async function runProjectOrPrintStartCommand(answers, userArguments) {
    ...
    if (!getShouldAutoStartFromArgs(userArguments)) {
        Logger.log(`To start the application run the following command:` +
            chalk.greenBright(`\n\ncd ${folderName}\n` + appRunScript));
        printDemoCoreWarning();
        return;
    }
